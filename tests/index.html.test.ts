...
test("test_remove_block_deletes_the_selected_static_block_and_all_incident_edges", async () => {
  await resetEditorFixture();
  // Step: select the fan-in terminal static block "Let it go" from the fixture.
  await selectEditorNode("B_LetGo");
  // Step: remove the block through the direct handler, not a DOM click.
  await runEditorAction("removeBlock");
  const source = await evaluate("codeBox.value");
  // Step: the declaration is gone, and no edge still points at the removed block.
  // (The autogenerated metadata trailer legitimately still names B_LetGo as the last-selected id, so this
  // checks the two real occurrences rather than a blanket !source.includes("B_LetGo").)
  assert.ok(!source.includes('B_LetGo["Let it go"]'));
  assert.ok(!source.includes("--> B_LetGo"));
  // Step: the remaining fixture source is saved to disk, parses with Mermaid, and reloads unchanged.
  await assertEditorSourceIsSavedAndValid();
});
